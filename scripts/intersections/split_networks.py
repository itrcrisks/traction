...
def main(networks_path, hazards_path, networks_csv, hazards_csv):
    ...
    hazard_transforms, transforms = read_transforms(hazards, hazards_path)
    ...
    for network_path in networks.path:
        fname = os.path.join(networks_path,"..",network_path)
        out_fname = os.path.join(
            networks_path,"..","..", "results", "hazard_asset_intersection",
        ...
        if "nodes" in layers:
            ...
            if os.path.exists(pq_fname_nodes):
                ...
                nodes = process_nodes(nodes, transforms, hazard_transforms, hazards_path)
        ...
        if "edges" in layers:
            ...
            if os.path.exists(pq_fname_edges):
                ...
                edges = process_edges(edges, transforms, hazard_transforms, hazards_path)
        ...
        if "areas" in layers:
            ...
            if os.path.exists(pq_fname_areas):
                ...
                areas = process_areas(areas, transforms, hazard_transforms, hazards_path)
...
def read_transforms(hazards, hazards_path):
    transforms = []
    transform_id = 0
    hazard_transforms = []
    for hazard in hazards.itertuples():
        hazard_path = hazard.path
        with rasterio.open(os.path.join(hazards_path, hazard_path)) as dataset:
            crs = dataset.crs
            width = dataset.width
...
def process_nodes(nodes, transforms, hazard_transforms, hazards_path):
    ...
    for hazard in hazard_transforms.itertuples():
        ...
        fname = os.path.join(hazards_path, hazard.path)
...
def process_edges(edges, transforms, hazard_transforms, hazards_path):
...
def process_areas(areas, transforms, hazard_transforms, hazards_path):
