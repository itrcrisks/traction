...
def main(damage_data_path, layers_path, processed_data_path, results_data_path, epsg_country):
    ...
    # print (param_values)
    with open(os.path.join(results_data_path, "parameter_combinations.txt"),"w+") as f:
    ...
    for param in param_values.itertuples():
        ...
        for asset_info in asset_data_details.itertuples():
            ...
            for hazard_file in hazard_data_files:
                ...
                hazard_data_details = pd.read_csv(os.path.join(layers_path,hazard_file),encoding="latin1")
                
                if os.path.isfile(hazard_intersection_file) is True: 
                    hazard_df = gpd.read_parquet(hazard_intersection_file)
                    hazard_df = hazard_df.to_crs(epsg=epsg_country)
